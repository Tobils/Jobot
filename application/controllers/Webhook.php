...
class Webhook extends CI_Controller {
  ...
  // UNTUK MEREPSON PESAN TEXT DARI USER
  ...
  //untuk merepons pesan sticker yg dikirim oleh user
  ...
  // untuk mengirim pertanyaan ke user
  ...
  private function checkAnswer($message, $replyToken)
  {
    ...
    if($this->user['number'] < 10)
    {
     ...
     $this->tebakkode_m->setUserProgress($this->user['user_id'], $this->user['number'] + 1);
      ...
      ß'Great! Mantap bro! Ketik "MULAI" untuk bermain lagi!';
      $textMessageBuilder2 = new TextMessageBuilder($message);
 
      // merge all message
      $multiMessageBuilder = new MultiMessageBuilder();
      $multiMessageBuilderß->add($textMessageBuilder1);
      $multiMessageBuilder->add($stickerMessageBuilder);
